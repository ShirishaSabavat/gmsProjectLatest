...
import {
  addPickupLocation, getGarages, getPickupLocation, editPickupLocation,
} from 'services/axios';
...
const createModules = () => {
  ...
  useEffect(() => {
    getPickupLocation(id)
      .then((res) => {
        ...
        setSelectedItem(res.data.results?.garageId);
  ...
  const onSave = (event) => {
    ...
    if (resp) {
      if (id !== -1) {
        console.log('in edit');
        editPickupLocation(pickupLocation, '', radioValue, dropDownMenu[selectedItem]?.id, id)
          .then((res) => {
            console.log('res', res);
          })
          .catch((err) => {
        ...
        console.log('in add');
        addPickupLocation(pickupLocation, '', radioValue, dropDownMenu[selectedItem]?.id)
          .then((res) => {
            console.log('res', res);
          })
          .catch((err) => {
  ...
  return (
    <div className="row px-4">
      <div className="col-12 py-3 px-4 bg-[#FFFFFF] mb-4">
        <h6 className="text-sm text-[#53565A] font-quicksand-semi-bold">Pickup Location</h6>
        <Input placeholder="Pickup Location" value={pickupLocation} onChange={(e) => setPickupLocationName(e.target.value)} style={{ backgroundColor: '#F5F8FC', borderColor: '#F5F8FC', padding: '8px' }} />
      ...
      <div className="col-12 py-3 px-4 bg-[#FFFFFF] mb-4">
        <h6 className="text-sm text-[#53565A] font-quicksand-semi-bold">Select Garage</h6>
          ...
          >
            <div className="row">
              <div span={22} className="col-6 text-start font-quicksand-medium">
                {dropDownMenu[selectedItem]?.name || 'Select from Garages(dropdown)'}
              </div>
